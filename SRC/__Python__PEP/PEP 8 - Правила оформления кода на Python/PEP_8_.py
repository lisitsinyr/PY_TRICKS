...
#beginfunction
    ...
    # * Если условная часть if-оператора настолько длинная, что её приходится записывать в несколько строк,
    # стоит отметить, что сочетание двухсимвольного ключевого слова (т. е. if),
    # одного пробела и открывающей скобки создаёт естественный отступ в 4 пробела
    # для последующих строк многострочного условного оператора.
    # Это может привести к визуальному конфликту с отступом вложенного в if-оператор блока кода,
    # который также естественным образом имеет отступ в 4 пробела.
    # В этом документе не указано,
    # как (и нужно ли) визуально отличать такие условные строки от вложенного набора внутри if-оператора.
    # Допустимые варианты в этой ситуации включают, помимо прочего, следующее: Без дополнительных отступов.
    if (this_is_one_thing and
        that_is_another_thing):
        pass

    # * Добавьте комментарий, который обеспечит некоторое различие в редакторах
    ...
    # Импорты всегда располагаются в верхней части файла, сразу после комментариев и строк
    # документации модуля и перед глобальными переменными и константами модуля.
    ...
    # Рекомендуется использовать абсолютный импорт, так как он обычно более читабелен и работает
    # лучше (или, по крайней мере, выдаёт более понятные сообщения об ошибках), если система импорта настроена неправильно (например, когда каталог внутри пакета оказывается в sys.path):
    ...
    # Следует избегать импорта с подстановочными знаками (from <module> import *)

    #======================================================
    # ## Названия модулей уровня Dunder
    #======================================================
    # «Дундеры» на уровне модуля (то есть имена с двумя ведущими и двумя завершающими символами подчеркивания), такие как __all__, __author__, __version__, и т. д., следует размещать после строки документации модуля, но перед любыми операторами импорта кроме from __future__ импорта из модуля.
    ...
    # # Строковые кавычки
    #======================================================
    # В Python строки в одинарных и двойных кавычках ничем не отличаются. В этом PEP нет рекомендаций на этот счёт. Выберите правило и придерживайтесь его. Однако если строка содержит одинарные или двойные кавычки, используйте другие кавычки, чтобы избежать обратных косых черт в строке.
    ...
    # * Напишите строки документации для всех общедоступных модулей, функций, классов и методов. Строки
    # документации не нужны для закрытых методов, но у вас должен быть комментарий, описывающий, что делает метод. Этот комментарий должен располагаться после строки def
    #
    ...
    ## Предписывающий: правила именования
    ...
    # Модули должны иметь короткие имена, состоящие только из строчных букв. В имени модуля можно использовать символы подчеркивания, если это улучшает читаемость. Пакеты Python также должны иметь короткие имена, состоящие только из строчных букв, хотя использование символов подчеркивания не рекомендуется.
    ...
    # Поскольку исключения должны быть классами, здесь применяется соглашение об именовании классов. Однако в названиях исключений следует использовать суффикс «Error» (если исключение действительно является ошибкой).
    ...
    # Если имя аргумента функции совпадает с зарезервированным ключевым словом, то, как правило, лучше добавить в конце символ подчеркивания, а не использовать сокращение или неправильное написание. Таким образом, class_ лучше, чем clss. (Возможно, лучше избегать таких совпадений, используя синонимы.)
    ...
    # Открытые атрибуты — это те атрибуты, которые, как вы ожидаете, будут использовать сторонние клиенты вашего класса, при этом вы обязуетесь избегать изменений, приводящих к обратной несовместимости. Закрытые атрибуты не предназначены для использования третьими лицами. Вы не даёте никаких гарантий, что закрытые атрибуты не изменятся или даже не будут удалены.
    ...
    # Примечание 1. Старайтесь, чтобы функциональное поведение не имело побочных эффектов, хотя такие побочные эффекты, как кэширование, обычно допустимы.
    ...
    # Любые гарантии обратной совместимости распространяются только на общедоступные интерфейсы. Соответственно, важно, чтобы пользователи могли чётко различать общедоступные и внутренние интерфейсы.
    ...
    # Импортированные имена всегда следует рассматривать как деталь реализации. Другие модули не должны полагаться на косвенный доступ к таким импортированным именам, если только они не являются явно задокументированной частью API содержащего модуля, например os.path или __init__-модуля пакета, который предоставляет функциональные возможности из подмодулей.
    ...
    # Например, не полагайтесь на эффективную реализацию конкатенации строк на месте в CPython для операторов в форме a += b или a = a + b. Эта оптимизация ненадёжна даже в CPython (она работает только для некоторых типов) и вообще отсутствует в реализациях, не использующих подсчёт ссылок. В частях библиотеки, чувствительных к производительности, вместо этого следует использовать форму ''.join(). Это обеспечит конкатенацию за линейное время в различных реализациях.
    ...
    # При намеренной замене внутреннего исключения (с помощью raise X from None) убедитесь, что соответствующие данные передаются в новое исключение (например, сохраняется имя атрибута при преобразовании KeyError в AttributeError или текст исходного исключения встраивается в сообщение нового исключения).
    #
    ...
    #
    # * При перехвате ошибок операционной системы лучше использовать явную иерархию исключений,
    # появившуюся в Python 3.3, а не интроспекцию значений errno
    # * Кроме того, во всех операторах try/except ограничьте try минимальным необходимым количеством
    #   кода. Это также позволяет избежать маскировки ошибок.
    ...
    # * Если ресурс локален для определённого участка кода, используйте оператор with, чтобы обеспечить
    # его быструю и надёжную очистку после использования. Также можно использовать оператор try/finally.
    #
    ...
    #
    # * Будьте последовательны в операторах return. Либо все операторы return в функции должны
    # возвращать выражение, либо ни один из них не должен этого делать. Если какой-либо оператор return возвращает выражение, то в операторах return, которые не возвращают значение, это должно быть явно указано как return None, а в конце функции (если это достижимо) должен присутствовать явный оператор return:
    ...
    # * Используйте ''.startswith() и ''.endswith() вместо срезов строк для проверки наличия префиксов или суффиксов.
    #
    # Функции startswith() и endswith() более удобны и менее подвержены ошибкам:
    ...
    # * При сравнении типов объектов всегда следует использовать функцию isinstance(), а не сравнивать
    # типы напрямую:
    ...
    #   игнорировать их. Однако ожидается, что пользователи сторонних библиотек захотят запускать средства проверки типов для этих библиотек. Для этого PEP 484 рекомендует использовать файлы-заглушки: файлы .pyi, которые считываются средством проверки типов вместо соответствующих файлов .py. Файлы-заглушки могут распространяться вместе с библиотекой или отдельно (с разрешения автора библиотеки) через репозиторий typeshed [5].
